  ...
  fetch("./data/pokemon/pokemon.json")
    ...
    .then((res) => {
      ...
      pokemones.forEach((pokemon) => {
        let tipo1 = (pokemon.type)
        let tipo2 = (pokemon.type)
        let bloque = document.createElement("div");
        bloque.id = pokemones.name;
        bloque.classList.add("bloquePokemones");
        bloque.classList.add(`pokemon-${pokemon.type[0]}`);
        bloque.innerHTML = `<div class="bloquetransparente" id="tarjeta_"> <h2 class="letrastarjetas" > ${pokemon.name}</h2> <img src="${pokemon.img}">
        <div> ${tipo1} </div>
         <h2 class="letrastarjetas">${pokemon.num} </div>`;
